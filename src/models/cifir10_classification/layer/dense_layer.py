...
class DenseLayer:
    def __init__(self, input_shape, hidden_dim, activation='relu', dropout=False,
        ...
        """
        :param input_shape: the input shape, [batch, input_size]
        :param hidden_dim:
        :param activation:
        :param dropout:
        :param keep_prob:
        ...
        self.weight = tf.Variable(
            initial_value=tf.random_normal(shape=[self.input_shape[1], self.hidden_dim],
                                           mean=0.0, stddev=np.sqrt(2.0/self.input_shape[1])),
            name="W_%s" % name
        )
        ...
        # 偏置向量
        self.bias = tf.Variable(initial_value=tf.constant(0.0, shape=[self.hidden_dim]),
                                name="b_%s" % name)
    ...
    def get_output(self, input):
        ...
        if self.batch_normal:
            ...
            self.hidden = tf.nn.batch_normalization(intermediate, mean=mean, variance=variance,
