# -*- coding: utf-8 -*-
"""
learn to use batch normalization layer
...
# training data
x = np.linspace(-7, 10, N_SAMPLES)[:, np.newaxis]    # shape(N_SAMPLES, 1)
np.random.shuffle(x)
noise = np.random.normal(loc=0, scale=2, size=x.shape)
y = np.square(x) - 5 + noise
train_data = np.hstack((x, y))    # shape(N_SAMPLES, 2)

# test data
test_x = np.linspace(-7, 10, 200)[:, np.newaxis]   # shape(200, 1)
noise = np.random.normal(loc=0, scale=2, size=test_x.shape)
test_y = np.square(test_x) - 5 + noise     # shape(200, 1)

# plot input data
plt.scatter(x=x, y=y, c='#FF9359', s=50, alpha=0.5, label='train')
...
# flag for using BN on training or testing
...
class NN(object):
    def __init__(self, batch_normalization=False):
        ...
        self.w_init = tf.random_normal_initializer(mean=0., stddev=.1)
        # if use normalization
        self.pre_activation = [tf_x]
        if self.is_bn:
            self.layer_input = [tf.layers.batch_normalization(inputs=tf_x, training=tf_is_train)]  # for input data
        else:
            self.layer_input = [tf_x]
        # adding hidden layers
        for i in range(N_HIDDEN):
            self.layer_input.append(self.add_layer(self.layer_input[-1], 10, activation=ACTIVATION))
        self.out = tf.layers.dense(inputs=self.layer_input[-1], units=1, kernel_initializer=self.w_init,
                                   bias_initializer=B_INIT)
        # compute loss
        self.loss = tf.losses.mean_squared_error(labels=tf_y, predictions=self.out)
    ...
    def add_layer(self, x, out_size, activation=None):
        """add a neural layer. (contain BN if use)"""
        x = tf.layers.dense(inputs=x, units=out_size, kernel_initializer=self.w_init, bias_initializer=B_INIT)
        ...
        out = x if activation is None else activation(x)
        return out
...
f, axs = plt.subplots(nrows=4, ncols=N_HIDDEN+1, figsize=(10, 5))
...
for epoch in range(EPOCH):
    ...
    in_epoch = True
    while in_epoch:
        # batch start index and end finish index
        b_s, b_f = (step*BATCH_SIZE) % len(train_data), ((step+1)*BATCH_SIZE) % len(train_data)
        step += 1
        if b_f < b_s:
            b_f = len(train_data)
            in_epoch = False    # util the data is done
        # batch training data
        b_x, b_y = train_data[b_s: b_f, 0:1], train_data[b_s: b_f, 1:2]
        # train
        sess.run([nets[0].train, nets[1].train], feed_dict={tf_x: b_x, tf_y: b_y, tf_is_train: True})
        ...
        if step == 1:
            ...
            # plot histogram
            # plot nets[0].layer_input, nets[1].layer_input, nets[0].pre_activation, nets[1].pre_activation
            plot_histogram(l_in, l_in_bn, pa, pa_bn)
